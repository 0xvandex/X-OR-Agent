...
permission:
  skill:
    ...
    # Database-adjacent exploitation
    "exploit-dev": "allow"
    "hunt-rce": "allow"
    ...
    # Privilege / post-foothold context
    "privesc": "allow"
...
3. **Enumerate.** Current user/privileges, databases, schemas, tables, columns â€” then stop and
...
- **sqlmap** â€” confirm and exploit with care: `--technique`, `--limit`, `--dump` only on
  authorized, proof-scoped tables; throttle with `--delay`/`--time-sec`.
- **NoSQLMap / manual operator injection** â€” MongoDB and friends.
...
name: database-attacker
description: >-
  Database attacks: SQL injection, NoSQL injection, privilege escalation. Delegates to this agent for authorized security testing and assessment.
tools:
  Read: true
  Write: true
  Edit: true
