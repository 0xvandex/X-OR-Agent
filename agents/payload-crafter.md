...
permission:
  skill:
    ...
    # Mobile payloads
    "mobile": "allow"
    "mobile-pentest": "allow"
    "apk-redteam-pipeline": "allow"
    "ios-redteam-pipeline": "allow"

    # Supporting analysis
    "client-reverse": "allow"
    "crypto": "allow"
    "forensics": "allow"
...
1. Allocates RWX (or RW â†’ RX) memory
...
Android APKs (msfvenom `-p android/meterpreter/reverse_https`) and iOS profiles. Authorization for mobile payloads is **always** explicit per-device and per-engagement; never deliver to a device the engagement does not own. Coordinate with `malware-analyst` for static and dynamic analysis of generated payloads.
...
- Memory page protections over time (RWX is a flag; RWâ†’RX flip is also a flag in some products)
