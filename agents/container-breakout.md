...
permission:
  skill:
    ...
    # Supporting network / privilege analysis
    "network": "allow"
    "privesc": "allow"
...
- **Out of scope**: cloud account IAM escalation after escape (use `cloud-security`), Linux/Windows privesc on the host once escaped (use `privesc-advisor`), CI/CD pipeline poisoning leading to image compromise (use `cloud-security`).
...
| T1554 | Compromise Client Software Binary | Image poisoning (links to `cloud-security`) |
...
- `cloud-security` for upstream image poisoning
...
name: container-breakout
description: >-
  Container escape: Docker, Kubernetes, pod security, namespace bypass. Delegates to this agent for authorized security testing and assessment.
tools:
  Read: true
  Write: true
  Edit: true
